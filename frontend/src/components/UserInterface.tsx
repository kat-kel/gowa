...
const UserInterface: React.FC<UserInterfaceProps> = ({ backendName }) => {
  ...
  const [newUser, setNewUser] = useState({ name: '', voice: '' });
  const [updateUser, setUpdateUser] = useState({ id: '', name: '', voice: '' });
      ...
      setUsers(
        users.map((user) => {
          if (user.id === parseInt(updateUser.id)) {
            return { ...user, name: updateUser.name, voice: updateUser.voice };
          }
          return user;
        })
      );
    ...
    <div className={`user-interface ${bgColor} ${backendName} w-full max-w-md p-4 my-4 rounded shadow`}>
      ...
      <form onSubmit={createUser} className="mb-6 p-4 bg-blue-100 rounded shadow">
        ...
        <input
          placeholder="voice"
          value={newUser.voice}
          onChange={(e) => setNewUser({ ...newUser, voice: e.target.value })}
          className="mb-2 w-full p-2 border border-gray-300 rounded"
        />
      ...
      <form onSubmit={handleUpdateUser} className="mb-6 p-4 bg-blue-100 rounded shadow">
        ...
        <input
          ...
          value={updateUser.voice}
          onChange={(e) => setUpdateUser({ ...updateUser, voice: e.target.value })}
          className="mb-2 w-full p-2 border border-gray-300 rounded"
        />
